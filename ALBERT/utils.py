...
def batchify_per_sentence(iterator, tokenizer, number_of_sentence, pretrained_albert, max_length=512):
    ...
    while sentence_count < n:
        ...
        token_count = len(tokenizer.tokenize(' '.join(iterator[sentence_count:stop])))
...
def batchify_per_sentence_with_context(iterator, tokenizer, number_of_sentence, number_sentence_before, pretrained_albert, max_length=512):
    """Batchify iterator sentence, to get batches of specified number of sentences.
    Arguments:
        - iterator: sentence iterator
        - number_of_sentence: int
    ...
    if number_sentence_before > 0:
        ...
        token_count = len(tokenizer.tokenize(iterator[stop]))
    ...
    while sentence_count < n:
        ...
        stop = min(sentence_count + number_of_sentence, n)
        token_count = len(tokenizer.tokenize(' '.join(iterator[start:stop])))
        while token_count > max_length:
            print('WARNING: decreasing number of sentence in a batch to fit max length of {}'.format(max_length))
            batch_modifications += 1
            stop -= 1
        ...
        indexes.append((len(tokenizer.tokenize(' '.join(iterator[start:start+number_sentence_before]))), len(tokenizer.tokenize(batch[-1]))))
        sentence_count = stop
    if batch_modifications > 0:
        print('WARNING: {} reductions were done when constructing batches... You should reduce the number of sentence to include.'.format(batch_modifications))
    return batch, indexes

def batchify_per_sentence_with_pre_and_post_context(iterator, tokenizer, number_of_sentence, number_sentence_before, number_sentence_after, pretrained_albert, max_length=512):
    """Batchify iterator sentence, to get batches of specified number of sentences.
    Arguments:
        - iterator: sentence iterator
        - number_of_sentence: int
    ...
    while sentence_count < n:
        ...
        token_count = len(tokenizer.tokenize(' '.join(iterator[start:stop_post_context])))
        while token_count > max_length:
            print('WARNING: decreasing number of sentence in a batch to fit max length of {}'.format(max_length))
            batch_modifications += 1
            stop -= 1
        ...
        indexes.append((len(tokenizer.tokenize(' '.join(iterator[start:start+number_sentence_before]))), len(tokenizer.tokenize(' '.join(iterator[start:stop])))))
        sentence_count = stop
    if batch_modifications > 0:
        print('WARNING: {} reductions were done when constructing batches... You should reduce the number of sentence to include.'.format(batch_modifications))
    return batch, indexes

def batchify(iterator, tokenizer, context_length, pretrained_albert, max_length=512):
...
def match_tokenized_to_untokenized(tokenized_sent, untokenized_sent, connection_character='▁', eos_token='[SEP]'):
